	...
	class Red_black_tree
	{
		...
		// METHODS
			...
			ft::pair<iterator, bool>	unique_insert (const value_type& val) 
			{
				...
				{
					tmp_parent = tmp;
					compareResult = _comp(val->first, tmp->getKeyOf() );
					// tmp = compareResult ? _S_left(__x): _S_right(__x);
				}//After the while, y refers to the parent node of the insertion point (it must be a leaf node at this time)
				...
				if (_comp(it.node->getKeyOf(), val->first) )
					return (ft::pair<iterator, bool>(insertNode(tmp, tmp_parent, val), true) );
			...
			iterator	insertNode (node_type *pos, node_type *pos_parent, const value_type& val)
			{
				...
				if (pos_parent == _header || !pos || _comp(val->first, pos_parent->getKeyOf()) )
				{
			...
			{
				node_type *head = _header;
				node_type *root = getRoot();

				while (root)
				{
					if (!_comp(root->getKey, key) )
						//  head = __x, __x = _S_left(__x);//Note the syntax
					else
						root = getRightChildOf(root);
				}
				// iterator it = iterator(head);   
				// return (it == end() || _M_key_compare(__k, _S_key(it._M_node)))? end(): it;
			}
		...
		protected:
			...
			NodeColour& getColorOf(node_type *node)
				{return node->colour; }
